...
class ExtractSyscoSubTotalTests(TestCase):
    """SUB TOTAL extraction from the Sysco totals box (2026-05-17).

    Used by parser.py to math-validate fee captures: when
    `SUB TOTAL + TAX == INVOICE TOTAL` exactly, the invoice has no
    non-tax fees and any CC/fuel captures are phantoms (from mid-page
    MISC CHARGES informational text). Drops those phantoms before
    they become synthetic ILI rows.
    """
    ...
    def test_extracts_sub_total_value(self):
        """SUB TOTAL + TOTAL labels at similar y, value in right column
        within ±0.02 y of (sub+total)/2."""
        import sys
        from django.conf import settings
        path = str(settings.BASE_DIR / 'invoice_processor')
        if path not in sys.path:
        ...
        T = self._tok
        # Mirror INV 775632629 page-4 totals box layout
        tokens = [
            T('SUB',     0.703, 0.809),
            T('TOTAL',   0.711, 0.822),
            T('2217.72', 0.802, 0.823),  # value to extract
            T('TAX',     0.705, 0.845),
            T('37.17',   0.812, 0.853),
            T('TOTAL',   0.714, 0.888),
            T('2254.89', 0.813, 0.883),
        ]
        pages = [{'tokens': tokens}]
        result = extract_sysco_sub_total(pages)
        self.assertEqual(result, 2217.72)

    def test_returns_none_when_no_sub_total(self):
        import sys
        from django.conf import settings
        path = str(settings.BASE_DIR / 'invoice_processor')
        if path not in sys.path:
        ...
        from section_validator import extract_sysco_sub_total
        T = self._tok
        # Just TAX + INVOICE TOTAL, no SUB TOTAL row
        tokens = [
            T('TAX',     0.705, 0.845),
            T('37.17',   0.812, 0.853),
            T('INVOICE', 0.700, 0.876),
            T('TOTAL',   0.714, 0.888),
            T('2254.89', 0.813, 0.883),
        ]
        pages = [{'tokens': tokens}]
